...
test("canSendNow проверяет дневной лимит, часовой, окно ожидания", () => {
  ...
  result = canSendNow({ now, campaign, sentTodayCount: 5, sentLastHourCount: 4, lastSentAt: now - 10 * 60_000 });
  assert.equal(result.ok, false);
...
test("nextOutboundDelay возвращает 3-25 мин", () => {
  const rng = () => 0.5;
  const d = nextOutboundDelay(rng);
  assert.ok(d >= 3 * 60_000 && d <= 25 * 60_000);
  const dMin = nextOutboundDelay(() => 0);
  const dMax = nextOutboundDelay(() => 0.9999);
  assert.equal(dMin, 3 * 60_000);
  assert.ok(dMax <= 25 * 60_000 && dMax >= 24 * 60_000);
});
