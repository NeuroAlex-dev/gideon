...
test("inbound: AI отвечает даже вне рабочих часов кампании (ответы НЕ ограничены working_hours)", async () => {
  const { db, cid, lead, conv } = setupFullAuto();
  // Кампания работает 9-22, но мы эмулируем входящее в 3 утра
  const sent = [];
  const proc = createInboundProcessor({
    db,
    askClaude: fakeAiReply,
  ...
  await proc.onInbound({ tgUserId: 11, tgUsername: "vasya", text: "пишу в 3 утра", tgMessageId: 1 });
  await new Promise((r) => setTimeout(r, 200));
  assert.equal(sent.length, 1, "AI должен ответить даже ночью — на ответы лидов рабочие часы НЕ применяются");
});
